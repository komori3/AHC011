...
namespace NBeam {
    ...
    struct State {
        ...
        inline void move(int dir) {
            int np = ep + d4[dir];
            cost -= cell_cost(np);
            hash ^= g_hash_table[ep][tiles[ep]]; hash ^= g_hash_table[np][tiles[np]];
...
namespace NPuzzle {
    ...
    struct State {
        ...
        void run_with_beam_search(int level, double duration) {
            ...
            NBeam::State bs(N - align_layer_size, /*2 * N * N * N - cmds.size()*/512, assign);
