...
input で与えられたタイルを用いて全域木を構築する方法で、偶数回の swap によって達成できるものを列挙して、手数の短いものを採用すればよさそう？（全域木の構築がまず難しそうではあるが…）

揃えるフェーズは [MM117 RotatingNumbers](https://togetter.com/li/1501039) とかを参考にすれば何とかなりそう

順位表を見ると maspy さんが 31.7M を出していて、25M を超えているので全域木を作っていることがわかる

---

16 種類のタイルで N^2-1 (N>=6) マスの盤面を埋めるとき、鳩の巣原理よりどれかのタイルは複数枚使うことになる

このとき、初期盤面と目標盤面の転倒数は同じ種類のタイルを交換することで自由に調節できるので、偶置換・奇置換を考慮しなくてよくなる（はず）

---

input で与えられたタイルを用いて木を作る方法を実装した

まず入力生成と同様の方法でランダムに全域木を作る

input で与えられたタイルの種類ごとの個数を配列にした target_ctr[16]  
ランダム全域木についてタイルの種類ごとの個数を配列にした tree_ctr[16]  
を用意する
...
山登りを多点スタートして所与のタイルで作れる候補の木を大量に生成して目標盤面とする

input と目標盤面のタイルを最大重みマッチング(Primal Dual)で対応付ける　コストはマンハッタン距離の総和
...
ビーム幅の調整を入れた
* N=6 で 15000, N=7 で 7000, それ以外は 5000
* N>=8 のケースでたまに事故るのはもう割り切るしかなさそう…

ルールベースで揃える際に linear conflict を考慮する
* 01-BFS を linear conflict 考慮した嘘ダイクストラに変更
* ビームサーチのコストに linear conflict を加味
